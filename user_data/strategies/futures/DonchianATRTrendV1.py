...
class DonchianATRTrendV1(IStrategy):
    INTERFACE_VERSION = 3
    ...
    adx_min_short = IntParameter(8, 30, default=14, space="buy")
    ...
    vol_ma_len_short = IntParameter(10, 80, default=24, space="buy")
    vol_k_short = DecimalParameter(0.5, 1.5, decimals=2, default=0.65, space="buy")
    ...
    atrp_min_short = DecimalParameter(0.0008, 0.03, decimals=4, default=0.0020, space="buy")
    atrp_max_short = DecimalParameter(0.02, 0.20, decimals=3, default=0.090, space="buy")
    ...
    # Risk & size
    risk_per_trade = DecimalParameter(0.003, 0.02, decimals=3, default=0.006, space="buy")  # equity fraction
    ...
    # --- Pullback & chase knobs (all hyperopt-able) ---
    # Pullback target relative to recent bar and ATR
    pb_pullback_bps = IntParameter(0, 200, default=30, space="buy")  # 0..200 bps => 0.00%..2.00% pullback
    pb_max_bps = IntParameter(10, 120, default=60, space="buy")  # cap from proposed price, in bps
    pb_atr_cap_mult = DecimalParameter(0.20, 1.20, decimals=2, default=0.80, space="buy")  # cap by ATR multiple

    # Which bar to reference for pullback (close/high/low)
    pb_ref = CategoricalParameter(["close", "high", "low"], default="low", space="buy")

    # Chasing behavior after signal has fired (to avoid missing fills)
    chase_enable = BooleanParameter(default=True, space="buy")
    chase_warmup_min = IntParameter(0, 60, default=10, space="buy")  # no chase for first N minutes
    chase_rate_bps_pm = IntParameter(2, 20, default=6, space="buy")  # bps per minute allowed drift
    chase_max_bps = IntParameter(10, 80, default=35, space="buy")  # absolute chase cap
    ...
    buy_params = {
        ...
        "adx_min_short": 24,
        "atrp_max": 0.04,
        "atrp_max_short": 0.028,
        "atrp_min": 0.0058,
        "atrp_min_short": 0.0162,
        "chase_enable": True,
        "chase_max_bps": 47,
        "chase_rate_bps_pm": 3,
        "chase_warmup_min": 34,
        "dc_atr_buf": 0.5,
        "dc_atr_buf_short": 0.5,
        "dc_len": 13,
        "ema200_slope_min": 0.0005,
        "ema200_slope_min_short": 0.0046,
        "htf_gate_mode": "ALL",
        "lev_round_step": 0.25,
        "lev_target_c": 0.113,
        "min_stake_usd": 80.0,
        "pb_atr_cap_mult": 1.16,
        "pb_max_bps": 115,
        "pb_pullback_bps": 190,
        "pb_ref": "close",
        ...
        "signal_boost_max": 1.17,
        "st_atr_period": 11,
        "st_break_eps_atr": 0.2,
        "st_multiplier": 1.7,
        "use_htf_ema": False,
        "use_htf_st": False,
        "user_max_leverage": 4,
    ...
    sell_params = {
        "activate_after_atr_mult": 3.2,
        "arm_max_minutes": 171,
        "atr_exhaust_mult": 0.84,
        "exit_atr_exhaust": False,
        "exit_ema_cross": False,
        "hard_sl_pct": -0.079,
        "initial_atr_mult": 1.48,
        "liq_buffer_mult": 3.3,
        "max_roi_cap": 0.15,
        "max_sl_cap": 0.074,
        "min_roi_floor": 0.014,
        ...
        "roi_atr_mult": 0.95,
        "time_decay_add": 0.006,
        "time_decay_minutes": 348,
        "trail_chandelier_mult": 3.08,
        "trail_roi_mult": 4.42
    }
    ...
    # -----------------------
    # Entry price with chasing (Oracle-like)
    # -----------------------
    def custom_entry_price(
        ...
        """
        Places a limit order using:
          • Pullback target (basis points) relative to a reference price (close/high/low) and capped by ATR multiple
          • Additional "chase" window that relaxes the target a few bps per minute (with warmup and max cap)
        Side-aware tweaks for shorts: smaller pullback and more aggressive chase to reduce missed fills.
        """
        # Fail safe
        if proposed_rate is None or proposed_rate <= 0:
            return None

        # Get latest analyzed row
        try:
        ...
        # --- Hyperopt knobs ---
        pb_bps = int(self.pb_pullback_bps.value)  # pullback in basis points
        max_bps = int(self.pb_max_bps.value)  # max deviation from proposed, in bps
        atr_mult = float(self.pb_atr_cap_mult.value)  # ATR cap multiple
        ref_key = str(self.pb_ref.value)

        chase_on = bool(self.chase_enable.value)
        warmup_min = int(self.chase_warmup_min.value)
        chase_rate = int(self.chase_rate_bps_pm.value)  # bps per minute
        chase_cap = int(self.chase_max_bps.value)

        # --- Side-aware adjustments (SHORTS get easier entries & more chase) ---
        if side == "short":
            # Keep pullback modest to avoid over-asking during fast drops
            pb_bps = min(pb_bps, 60)
            # Ensure chase is enabled and reasonably assertive
            chase_on = True
            warmup_min = min(warmup_min, 10)
            chase_rate = max(chase_rate, 12)
            chase_cap = max(chase_cap, 80)

        # Basis-point helpers
        def bps_to_frac(bps: int) -> float:
            return max(0.0, bps) / 10000.0

        pullback = bps_to_frac(pb_bps)
        max_dev = bps_to_frac(max_bps)
        atr_cap = atr_mult * atr  # absolute price distance cap (in price units)

        # Choose reference
        ref_price = {"close": close, "high": high, "low": low}.get(ref_key, low)

        # --- Base pullback target (before chasing) ---
        if side == "long":
            # Aim *below* reference, but not more than max_dev below proposed
            target_pb = ref_price * (1.0 - pullback)
            target_cap1 = proposed_rate * (1.0 - max_dev)
            target_cap2 = ref_price - atr_cap if atr_cap > 0 else target_pb  # not below ref - ATR*mult
            baseline = max(target_pb, target_cap1, target_cap2)
            # Never more optimistic than proposed (we won't try to buy cheaper than exchange would accept instantly)
            base_limit = min(baseline, proposed_rate)
        elif side == "short":
            # Aim *above* reference for shorts
            target_pb = ref_price * (1.0 + pullback)
            target_cap1 = proposed_rate * (1.0 + max_dev)
            target_cap2 = ref_price + atr_cap if atr_cap > 0 else target_pb  # not above ref + ATR*mult
            baseline = min(target_pb, target_cap1, target_cap2)
            base_limit = max(baseline, proposed_rate)
        else:
            return float(proposed_rate)

        # --- Chasing window (relax toward market after some time) ---
        if chase_on:
            try:
                sig_col = "enter_long" if side == "long" else "enter_short"
                sig_times = df.index[df[sig_col] == 1]
                minutes_since_signal = int((current_time - sig_times[-1]).total_seconds() // 60) if len(
                    sig_times) else 0
            except Exception:
                minutes_since_signal = 0

            # No chase before warmup
            eff_minutes = max(0, minutes_since_signal - warmup_min)
            if eff_minutes > 0:
                allowed_bps = min(chase_cap, chase_rate * eff_minutes)
                allowed_frac = bps_to_frac(allowed_bps)

                if side == "long":
                    # Move price up toward market by up to allowed_frac of *current close*
                    chase_floor = close * (1.0 - allowed_frac)
                    base_limit = max(base_limit, chase_floor)
                else:
                    chase_ceiling = close * (1.0 + allowed_frac)
                    base_limit = min(base_limit, chase_ceiling)

        # --- Optional: round to exchange tick/step to reduce rejects ---
        try:
            mkt = self.dp.market(pair)
            price_step = float(mkt.get("price_step") or mkt.get("tick_size") or 0.0)
            if price_step and price_step > 0:
                if side == "long":
                    # round UP a tiny bit so order is valid and more likely to fill
                    rounded = np.ceil(base_limit / price_step) * price_step
                else:
                    # round DOWN a tiny bit for shorts
                    rounded = np.floor(base_limit / price_step) * price_step
