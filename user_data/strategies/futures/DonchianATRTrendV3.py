...
class DonchianATRTrendV3(IStrategy):
    # Core settings
    timeframe = "1h"
    can_short = True
    ...
    # Regime / breakout quality filters
    width_atr_min = DecimalParameter(1.0, 2.5, decimals=2, default=1.3, space="buy", optimize=True)
    st_flip_lb = IntParameter(1, 3, default=2, space="buy", optimize=True)
    retest_atr = DecimalParameter(0.15, 0.60, decimals=2, default=0.25, space="buy", optimize=True)
    slope_min = DecimalParameter(0.0002, 0.0020, decimals=4, default=0.0006, space="buy", optimize=True)
    alpha_break = DecimalParameter(0.08, 0.30, decimals=2, default=0.12, space="buy", optimize=True)
    ...
    # --- Custom exits ---
    custom_exit_flag = BooleanParameter(default=True, space="sell", optimize=True)
    fbo_bars = IntParameter(1, 3, default=2, space="sell", optimize=True)
    fbo_atr = DecimalParameter(0.15, 0.45, decimals=2, default=0.25, space="sell", optimize=True)
    tstop_bars = IntParameter(12, 36, default=16, space="sell", optimize=True)
    tstop_floor = DecimalParameter(0.000, 0.012, decimals=3, default=0.004, space="sell", optimize=True)
    ...
    sell_params = {
        "activate_after_atr_mult": 3.4,
        "arm_max_minutes": 60,
        "custom_exit_flag": False,
        "fbo_atr": 0.38,
        "fbo_bars": 3,
        "hard_sl_pct": -0.049,
        "initial_atr_mult": 3.25,
        "liq_buffer_mult": 4.5,
        "max_sl_cap": 0.047,
        "mmr_est": 0.01,
        "profit_lock_trigger": 0.015,
        "roi_decay_minutes": 55,
        "roi_floor": 0.009,
        "roi_start": 0.015,
        "sl_floor": 0.034,
        "trail_chandelier_mult": 7.1,
        "tstop_bars": 20,
        "tstop_floor": 0.005,
    }
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Donchian channel (1h-appropriate window set by self.buy_dc_period)
        ...
        # Regime features (width vs ATR, midline slope)
        df["dc_width_prev"] = df["dc_upper_prev"] - df["dc_lower_prev"]
        df["width_atr_ratio"] = df["dc_width_prev"] / df["atr"].replace(0, np.nan)
        df["dc_mid_slope"] = (df["dc_mid_prev"].diff().abs()) / df["close"].replace(0, np.nan)

        # “Fresh” ST flip window
        flip_up = ((df["st_trend"] == 1) & (df["st_trend"].shift(1) == -1)).astype(int)
        flip_dn = ((df["st_trend"] == -1) & (df["st_trend"].shift(1) == 1)).astype(int)
        lb = int(self.st_flip_lb.value)
        df["st_up_recent"] = flip_up.rolling(lb, min_periods=1).max().astype(bool)
        df["st_down_recent"] = flip_dn.rolling(lb, min_periods=1).max().astype(bool)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        long_cond = [
            df["close"] > df["dc_upper_prev"] + float(self.alpha_break.value) * df["atr"],
            df["st_trend"] == 1,
            df["st_up_recent"],
            df["width_atr_ratio"] >= float(self.width_atr_min.value),
            df["dc_mid_slope"] >= float(self.slope_min.value),
            df["low"] <= df["dc_upper_prev"] + float(self.retest_atr.value) * df["atr"],
            df["volume"] > 0,
        ]
        ...
        short_cond = [
            df["close"] < df["dc_lower_prev"] - float(self.alpha_break.value) * df["atr"],
            df["st_trend"] == -1,
            df["st_down_recent"],
            df["width_atr_ratio"] >= float(self.width_atr_min.value),
            df["dc_mid_slope"] >= float(self.slope_min.value),
            df["high"] >= df["dc_lower_prev"] - float(self.retest_atr.value) * df["atr"],
            df["volume"] > 0,
        ]
        if bool(self.buy_confirm_mid.value):
            short_cond.append(df["close"] < df["dc_mid_prev"])

        df.loc[np.logical_and.reduce(long_cond), "enter_long"] = 1
        df.loc[np.logical_and.reduce(short_cond), "enter_short"] = 1
    ...
    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        if not bool(self.custom_exit_flag.value):
            return None
        ...
        since = df.loc[df["date"] >= pd.Timestamp(trade.date_entry_fill_utc)]
        if since.empty:
            return None

        bars = max(0, len(since) - 1)
        last = df.iloc[-1]

        # 1) Failed-breakout (first N bars revert inside channel by > fbo_atr*ATR)
        if not trade.is_short and bars <= int(self.fbo_bars.value):
            if last["close"] < (last["dc_upper_prev"] - float(self.fbo_atr.value) * last["atr"]):
                return "FBO_long"
        if trade.is_short and bars <= int(self.fbo_bars.value):
            if last["close"] > (last["dc_lower_prev"] + float(self.fbo_atr.value) * last["atr"]):
                return "FBO_short"

        # 2) Time-stop with profit floor
        if bars >= int(self.tstop_bars.value) and current_profit < float(self.tstop_floor.value):
            return "TimeStop"

        # 3) Supertrend flip
        if not trade.is_short and last["st_trend"] == -1:
            return "ST_flip_long"
        if trade.is_short and last["st_trend"] == 1:
            return "ST_flip_short"
    ...
    def custom_stoploss(self, pair: str, trade: Any, current_time: datetime,
        ...
        try:
            ...
            since_entry = df.loc[df["date"] >= pd.Timestamp(trade.date_entry_fill_utc)]
