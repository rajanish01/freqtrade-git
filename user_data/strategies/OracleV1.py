...
class OracleV1(IStrategy):
    """
    OracleV52 – volatility-aware upgrades to OracleV51

    Key changes:
    - Adaptive RSI margin: base + k * normalized ATR%
    - Side-specific cooldowns (independent for long/short)
    - Dynamic dip lookback based on ATR% regime
    - Extra exits: EMA crossback + ATR exhaustion (vol collapse)
    - Profit lock to breakeven once profit crosses a threshold
    - Preserves ATR%-activated trailing + Chandelier blending
    - Preserves leverage sizing, dynamic stake based on risk
    """
    INTERFACE_VERSION = 3
    timeframe = "1h"
    can_short = True
    ...
    buy_params = {
        "band_overshoot_min": 0.017,
        "bb_shift": 1.07,
        "buy_rsi_pct": 32,
        "dip_lookback": 7,
        "dip_lookback_hi": 4,
        "dip_lookback_lo": 9,
        "dyn_dip_atr_q_high": 0.73,
        "dyn_dip_atr_q_low": 0.36,
        "enable_reclaim": True,
        "entry_atr_cap": 1.15,
        "entry_cooldown_bars_long": 6,
        "entry_cooldown_bars_short": 8,
        "entry_pullback_max": 0.012,
        "lev_round_step": 0.05,
        "lev_target_c": 0.14,
        "min_atr_pct": 0.01,
        "min_stake_usd": 185.0,
    ...
    sell_params = {
        "activate_after_atr_mult": 3.6,
        "arm_max_minutes": 53,
        "atr_exhaust_mult": 0.57,
        "exit_atr_exhaust": True,
        "exit_ema_cross": True,
        "hard_sl_pct": -0.098,
        "initial_atr_mult": 1.88,
        "liq_buffer_mult": 6.6,
        "max_roi_cap": 0.14,
        "max_sl_cap": 0.077,
        "min_roi_floor": 0.01,
        "min_sl_floor": 0.011,
        "mmr_est": 0.004,
        "profit_lock_trigger": 0.019,
        "roi_atr_mult": 1.87,
        "time_decay_add": 0.013,
        "time_decay_minutes": 462,
        "trail_chandelier_mult": 4.76,
        "trail_roi_mult": 7.62,
    }

    # ====== Hyperoptables (new + existing) ======
    ...
    risk_per_trade = DecimalParameter(0.005, 0.025, decimals=3, default=0.012, space="buy")
    ...
    def populate_indicators(self, df: pd.DataFrame, metadata: Dict) -> pd.DataFrame:
        ...
        # Precompute ATR% quantiles for dynamic lookback selection
    ...
    def populate_entry_trend(self, df: pd.DataFrame, metadata: Dict) -> pd.DataFrame:
        ...
        # Adaptive RSI margin (vol-aware)
        rsi_margin = float(self.rsi_margin_base.value) + float(self.rsi_margin_k.value) * (
                df["atr_pct"].fillna(0) * 100.0)
        ...
        # Dynamic dip lookback regime flags (we will NOT pass Series into .rolling())
        ...
        if bool(self.enable_reclaim.value):
            ...
            # Precompute rolling(max) for 3 fixed windows, then pick per row
    ...
    def populate_exit_trend(self, df: pd.DataFrame, metadata: Dict) -> pd.DataFrame:
        ...
        # ATR exhaustion: if current ATR% collapses vs median/rolling typical → exit (optional)
        if bool(self.exit_atr_exhaust.value):
            # median ATR% over a moderate window as a "typical" reference
    ...
    # ====== Entry price refinement (unchanged logic) ======
    def custom_entry_price(
        ...
        if side == "long":
            ...
            target = max(raw_target, floor_pct, floor_atr)
            return min(target, proposed_rate)
            ...
            target = min(raw_target, cap_pct, cap_atr)
            return max(target, proposed_rate)

        return None

    # ====== Custom stoploss (adds profit lock to breakeven) ======
    def custom_stoploss(
            self,
            pair: str,
            trade: Any,
        ...
        # Profit lock: once profit >= trigger, never let stop go below breakeven
        lock_trig = float(self.profit_lock_trigger.value)
        lock_to_breakeven = current_profit is not None and current_profit >= lock_trig
        breakeven_dist = 0.0  # as a fraction (0% below current price is allowed in Freqtrade by converting to negative later)

        if not activated:
            early_dist = max(min_floor, dynamic_dist)
            if lock_to_breakeven:
                # respect the tighter of dynamic vs breakeven (breakeven means 0% below entry; translate to minimal stop)
                early_dist = max(min_floor, min(early_dist, 0.000001))
        ...
        # Blend: tighter of dynamic vs CE, respect floor (and breakeven lock if armed)
        final_dist = max(min_floor, min(dynamic_dist, ce_dist))
        if lock_to_breakeven:
            final_dist = max(min_floor, min(final_dist, breakeven_dist + 1e-6))

        return -float(final_dist)

    # ====== Custom ROI (adds time decay) ======
    def custom_roi(
            self,
            pair: str,
            trade: Any,
        ...
        # Base floor from entry ATR%
        try:
            entry_idx = df.index.get_loc(trade.date_entry_fill_utc)
    ...
    def custom_stake_amount(
        ...
        if min_stake: stake = max(stake, float(min_stake))
        if max_stake: stake = min(stake, float(max_stake))
        if stake < float(self.min_stake_usd.value):
            return 0.0
        return float(stake)

    # ====== Leverage (unchanged from original logic) ======
    def leverage(
            self,
            pair: str,
            current_time: datetime,
        ...
        boost = 1.0 + (self._signal_strength(df, side) * (float(self.signal_boost_max.value) - 1.0))
        L *= boost
